...
int  UnitTest::run()
{
	...
	for (n = 1, i = testcases.begin() ; i != testcases.end() ; ++i, ++n)
	{
		...
		try {
			...
			std::cerr << e.what() << AC_RED "FATAL: Assertion exception during setUp!" AA_RESET << std::endl;
			++exceptionCount;
			continue; // skip this test
		} catch(const std::exception& e) {
			std::cerr << e.what() << AC_RED "FATAL: Exception during setUp!" AA_RESET << std::endl;
			++exceptionCount;
			continue; // skip this test
		} catch(...) {
		...
		try {
			...
			if(exceptionExpected) {		// was expected
				std::cerr << e.what() << AC_GREEN "GOOD: An expected assertion exception occurred" AA_RESET << std::endl;
			} else {			    // was not expected
				std::cerr << e.what() << AC_RED "FAIL: UNEXPECTED ASSERTION EXCEPTION!" AA_RESET << std::endl;
				++exceptionCount;
			}
		} catch(const std::exception& e) {
			if(exceptionExpected) {		// was expected
				std::cerr << e.what() << AC_GREEN "GOOD: An expected exception occurred" AA_RESET << std::endl;
			} else {			    // was not expected
				std::cerr << e.what() << AC_RED "FAIL: UNEXPECTED EXCEPTION!" AA_RESET << std::endl;
				++exceptionCount;
			}
		} catch(...) {
			if(exceptionExpected) {		// was expected
		...
		try {
			...
			std::cerr << e.what() << AC_RED "FATAL: Assertion exception during tearDown!" AA_RESET << std::endl;
			++exceptionCount;
		} catch(const std::exception& e) {
			std::cerr << e.what() << AC_RED "FATAL: Exception during tearDown!" AA_RESET << std::endl;
			++exceptionCount;
		} catch(...) {
