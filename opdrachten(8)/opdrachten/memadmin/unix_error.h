 ...
 * include "here.h" and prepend "__HERE__" to the message.
@code
try {
	// With C++ STL classes
	ifstream  inputfile("some_filename");	// create an ifstream
	if (!inputfile)							// failed to open?
		throw unix_error(__HERE__,"inputfile");	// report the problem

	// With unix systemcalls
	if (chdir("/tmp") < 0)					// failed to change directory?
		throw unix_error(__HERE__,"chdir /tmp");// report the problem
}
@endcode
 */
...
public:
	...
	/// @param what		The additional text
	unix_error(const std::string& where, const char *what);

	/// Construct a unix system error with some additional text
	/// @param where	Where the error occured
	/// @param what		The additional text
	unix_error(const std::string& where, const std::string& what);
